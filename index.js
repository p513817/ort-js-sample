...
async function imgSet() {
  ...
  // let imageData = await WebDNN.Image.getImageArray(document.getElementById("video").value, {dstW: 224, dstH: 224});
  // WebDNN.Image.setImageArrayToCanvas(imageData, 224, 224, document.getElementById('video'));
...
async function runExample() {
  ...
  const frame = await captureCam();
  const imageData = await imageLoader.getImageData(frame);
...
function stopInfer() {
  console.log('Stop Interval: ', inter);
  clearInterval(inter);
}
/**
